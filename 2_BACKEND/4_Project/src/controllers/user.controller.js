...
const loginUser = asyncHandler(async (req, res) => {
  ...
  const { accessToken, refreshToken } = await generateAccessAndRefreshToken(user._id); //generate tokens of present user
...
const logoutUser = asyncHandler(async (req, res) => {
  await User.findByIdAndUpdate(    //using verifyJWT, user is added in req
  ...
  return res.status(200)   //return success code
